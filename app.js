...
async function main() {
	// const obj = {"ID":"item-838","Name":"goodday","Owner":"org1","Price":"100","Quantity":"10","asset_properties":{"Price":"90","asset_id":"item-838","object_type":"asset_properties","salt":"3738"}};
...
// console.log(obj.ID,obj.Price);
	...
	try {
		...
		try {
			...
			await contract1Org1.submitTransaction('InitLedger');
			console.log('*** Result: committed');
			} catch (createError) {
				console.log(`${RED}<-- Submit Failed: InitLedger - ${createError}${RESET}`);
			}
			try {
				// Addbalance
				console.log(`${GREEN}--> Submit Transaction: AddBalance`);
				transaction = contract1Org1.createTransaction('AddBalance');
				const res = await transaction.submit('org1',200);
				console.log(res.toString());
				console.log(`${GREEN}<-- Submit AddBalance Result: committed`);
			} catch (createError) {
				console.log(`${RED}<-- Submit Failed: AddBalance - ${createError}${RESET}`);
			}
			// try {
			// 	// Get balance
			// 	console.log(`${GREEN}--> Evaluate: Getbalance org1`);
			// 	const resultBuffer = await contract1Org1.evaluateTransaction('GetBalance', 'org1');
			// 	console.log(resultBuffer.toString());
			// 	// checkAsset(org1, resultBuffer, '1000');

			// } catch (readError) {
			// 	console.log(`${RED}<-- Failed: Getbalance - ${readError}${RESET}`);
			// }
			// try {
			// 	// Get balance
			// 	console.log(`${GREEN}--> Evaluate: Getbalance, - ${assetKey} should be owned by Sam${RESET}`);
			// 	const resultBuffer = await contract1Org1.evaluateTransaction('GetBalance', 'org1');
			// 	console.log(resultBuffer.toString());
			// 	// checkAsset(org1, resultBuffer, '1200');
			...
			try {
				// C R E A T E
				console.log(`${GREEN}--> Submit Transaction: AddItem, ${assetKey} owned by Sam${RESET}`);
				transaction = contract2Org1.createTransaction('AddItem');
				...
				await transaction.submit(assetKey, 'goodday', '10', 'org1', '100');
			...
			try {
				// R E A D
				console.log(`${GREEN}--> Evaluate: Get Item, - ${assetKey} should be owned by Sam${RESET}`);
				const resultBuffer = await contract2Org1.evaluateTransaction('GetItem', assetKey);
				console.log(resultBuffer.toString());
			...
			try{
				// Addtomarket place
				console.log('Adding to market place\n');
				const res =	await contract2Org1.submitTransaction('AddToMarketPlace', assetKey);
				// transaction = contract2Org1.createTransaction('AddToMarketPlace');
				// const res = await transaction.submit(assetKey);
				console.log(res);
				console.log('Done \n\n\n\n\n');
			}catch (readError) {
				console.log(`${RED}<-- Failed: Adding to market place - ${readError}${RESET}`);
			}
			await sleep(5000);
			try{
				console.log(`${GREEN}--> Evaluate: Get Items in Market, `);
				const resultBuffer = await contract2Org1.evaluateTransaction('GetItemsInMarket', assetKey);
				console.log(resultBuffer.toString());
			}catch (readError) {
				console.log(`${RED}<-- Failed: Getting Market Place items - ${readError}${RESET}`);
			}
			try{
				// Buy from market place
				console.log('Buying from market place\n');
				const res =	await contract2Org1.submitTransaction('BuyFromMarket', 'item-16','org2');
				// transaction = contract2Org1.createTransaction('AddToMarketPlace');
				// const res = await transaction.submit(assetKey);
				console.log(res);
				console.log('Done \n\n\n\n\n');
			}catch (readError) {
				console.log(`${RED}<-- Failed: Buying from market place - ${readError}${RESET}`);
			}
			await sleep(5000);
			try{
				console.log(`${GREEN}--> Evaluate: Get Items in Market, `);
